...
const babel = (outDir, envName) =>
  shell(
    `npx babel ${srcRoot} -x .es6,.js,.es,.jsx,.mjs,.ts,.tsx --out-dir ${outDir} --env-name "${envName}"`,
  );
...
Promise.resolve(true)
  ...
  .catch((err) => {
    if (err) console.error(red(err.stack || JSON.stringify(err)));
    process.exit(1);
  });
